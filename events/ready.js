module.exports = async client => {
  client.logger.log(`User: ${client.user.tag} | Prefix: ${process.env.prefix} | Serving ${client.users.cache.size} users in ${client.guilds.cache.size} server${client.guilds.cache.size === 1 ? '' : 's'}`, 'ready');
  client.user.setActivity(`${process.env.prefix}help | ${client.guilds.cache.size} guild${client.guilds.cache.size === 1 ? '' : 's'}`, { type: 'LISTENING' });
