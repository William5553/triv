...
exports.run = async (client, message, args) => {
  ...
  if (userr.permissions.has("BAN_MEMBERS"))
    return message.reply(
      "the person you tried to ban is too op (they also have the ban members permission)"
    );
  ...
  await userr.user.send(
    `you've been banned from ${message.channel.guild.name}`
  );
  const reason =
